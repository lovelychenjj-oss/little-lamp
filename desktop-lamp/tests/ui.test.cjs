...
function lamp(){const html=fs.readFileSync(path.join(root,'index.html'),'utf8'),nodes={};for(const m of html.matchAll(/id="([^"]+)"/g))nodes[m[1]]=element();const sent=[];const window={webkit:{messageHandlers:{lamp:{postMessage:m=>sent.push(m)}}}};vm.runInNewContext(fs.readFileSync(path.join(root,'catalog.js'),'utf8')+'\n'+fs.readFileSync(path.join(root,'lamp.js'),'utf8'),{window,document:{getElementById:id=>nodes[id]},setTimeout:()=>1,clearTimeout(){}});return {nodes,sent,window};}
